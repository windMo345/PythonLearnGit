...
import asyncic, logging, aiomysql
...
class ModelMetaclass(type):
	...
	def __new__(cls, name, bases, attrs):
		...
		sttrs['__fields__'] = fields  #类属性__fields__记录除主键外的其他属性
		#用于增删改查的sql语句(下面的赋值占位符用'?'是因为cursor.execute函数需要两个参数,一个是带赋值占位符的sql语句,另一个参数是占位符对应的值组,所以sql语句在编译时是没有对应值组的,换成'?'就不会报错了)
		attrs['__select__'] = 'select `%s`,%s from `%s`' % (primaryKey, ','.jion(list(map(lambda f: '`%s`' % mappings.get(f).name or f, fields))), tableName)  #类属性__select__记录查询语句
		attrs['__insert__'] = 'insert into '%s' (%s,`%s`) values (%s)' % (','.jion(list(map(lambda f: '`%s`' % mappings.get(f).name or f, fields))), primaryKey, create_args_string(len(fields)+1))  #类属性__insert__记录增添语句
		attrs['__update__'] = 'update `%s` set %s where `%s` = ?' % (tableName, ','.jion(list(map(lambda f: '`%s`=?' % mappings.get(f).name or f, fields))), primaryKey)  #类属性__update__记录修改语句
...
class Model(dict, metaclass = ModelMetaclass):
		...
		if value is None:
			...
			if field is not None:
				if field.default is not None:
					...
					logging.info('using default value for %s:%s' % (key, str(vale)))  #%s占位符只能对应字符串类型数据
	...
	async def findAll(cls, where=None, args=None, **kw):
		...
		rs = await select(' '.jion(sql), args)  #执行查询语句并等待查询结果
	...
	async def findNumber(cls, selectField, where=None, args=None)
		...
		rs = await select(' '.jion(sql), args, 1)  #执行查询语句并等待查询结果
		if len(rs) == 0:
			return None
